...
# Keyword-based mock responses for Phase 1
MOCK_RESPONSES = {
...
def send_message(session_id: str, req: MessageRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Pick response based on keyword
...
def end_session_and_analyze(session_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if client:
        ...
        client.from_("chat_sessions").update({"status": "ended"}).eq("id", session_id).execute()
        # Save session analysis
        client.from_("session_analysis").upsert([analysis_payload]).execute()
